        ...
        if (strings.length == 4) {
            if (CurrencyHandler.isCurrencyExist(strings[2])) {
                commandSender.sendMessage(MessageConstants.MESSAGE_PREFIX +
                        MessageConstants.SET_CURRENCY_TO_PLAYER.replace("{player}", strings[1])
                                .replace("{currency}", strings[2])
                                .replace("{amount}", strings[3]));
                CurrencyHandler.setCurrencyToPlayer(strings[1], strings[2], Integer.parseInt(strings[3]));
            } else {
                commandSender.sendMessage(MessageConstants.MESSAGE_PREFIX +
                        MessageConstants.CURRENCY_NOT_EXIST.replace("{currency}", strings[2]));
            }
        } else {
            commandSender.sendMessage(MessageConstants.MESSAGE_PREFIX + MessageConstants.SET_COMMAND_TIP);
        }
        return true;
    }
}
